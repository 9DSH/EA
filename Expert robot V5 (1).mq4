...
string        GridName = "Expert robot";       // identifies the grid. allows for several co-existing grids
extern bool   LimitOrder   =  false;
extern bool   StopOrder    =  true;
...
extern double  ProfitInUSD = 30;
extern double  LossInUSD = 20;
extern double  NetProfitInUSD = 10;
...
//----
//   GridName = StringConcatenate( "Expert robot", Symbol() );
   return(0);
  }
//+------------------------------------------------------------------------+
...
//---- 
   ...
   if(AccountNumber() == 12345678){
      ...
      Alert("Your account number is not valid. Please contact \"debasisrathforex@gmail.com\" for validation.");
      Sleep(22000000);
      return(0);
   }
...
//   }
   ...
   for (i=0; i<OrdersTotal(); i++) {                                               
      if (OrderSelect(i,SELECT_BY_POS,MODE_TRADES)==true) {
         if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumberGrid && OrderType() == OP_BUY) {
            double profit  = OrderProfit()+OrderCommission()+OrderSwap();
            int ticket0 = OrderTicket();
            double tLots0 = OrderLots();
            if(profit >= ProfitInUSD) {
            
               for (int j=0; j<OrdersTotal(); j++) {                                               
                  if (OrderSelect(j,SELECT_BY_POS,MODE_TRADES)==true) {
                     if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumberGrid && OrderType() == OP_SELL) {
                       
                        double profit2 = OrderProfit()+OrderCommission()+OrderSwap();
                        double ticket02 = OrderTicket();
                        double tLots02 = OrderLots();
                        if(profit2 <= (-LossInUSD) && (profit+profit2) >= NetProfitInUSD) {
                        
                           bool chk = OrderClose(ticket0,tLots0,Bid,Slippage,Blue);
                           bool chk1 = OrderClose(ticket02,tLots02,Ask,Slippage,Red);
                     //      bool chk = OrderCloseBy(ticket0,ticket02,White);
                           if(!chk) Print("Error in close1: "+GetLastError());
                           if(!chk1) Print("Error in close2: "+GetLastError());
                           else Print("Trade closed by Total Net Profit in USD.  Buy order no. "+ticket0+"  Sell order no. "+ticket02); 
                        }
                     }
                  }
               }
               ...
               for (j=0; j<OrdersTotal(); j++) {                                               
                  if (OrderSelect(j,SELECT_BY_POS,MODE_TRADES)==true) {
                     if(OrderSymbol() == Symbol() && OrderMagicNumber() == MagicNumberGrid && OrderType() == OP_BUY) {
                        ...
                        ticket02 = OrderTicket();
                        tLots02 = OrderLots();
                        if(profit2 <= (-LossInUSD) && (profit+profit2) >= NetProfitInUSD) {
                        
                           chk = OrderClose(ticket0,tLots0,Ask,Slippage,Red);
                           chk1 = OrderClose(ticket02,tLots02,Bid,Slippage,Blue);
                           
                           //chk = OrderCloseBy(ticket0,ticket02,White);
                           if(!chk) Print("Error in close1: "+GetLastError());
                           if(!chk1) Print("Error in close2: "+GetLastError());
                           else Print("Trade closed by Total Net Profit in USD.  Sell order no. "+ticket0+"  Buy order no. "+ticket02); 
                        }
                     }
                  }
               }
         ...
         if ( ProfitRank1 && ((CheckPositionRank & 1) == 0) )           // test if i have no open orders profit rank 1 close to my price: if so, put one on
          {
            ...
            if( ((traderate_Long > Ask ) && ((traderate_Long-Ask) >= GridSize*Point) && (wantBreakout)) || ((traderate_Long <= Ask ) && ((Ask-traderate_Long) >= GridSize*Point) && (wantCounter)) ) 
               {
                  
                  if(entermode == OP_BUYSTOP && StopOrder) {
                     ...
                     Print("In First BuyStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long);
                  }
                  if(entermode == OP_BUYLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,LongLots,traderate_Long,0,myStopLoss,traderate_Long+point*BaseTakeProfit,GridName,MagicNumberGrid,0,Green); 
                     Print("In First BuyLimit.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long);
                  }
               }
          }
            ...
            if( ((traderate_Long > Ask ) && ((traderate_Long-Ask) >= GridSize*Point) && (wantBreakout)) || ((traderate_Long <= Ask ) && ((Ask-traderate_Long) >= GridSize*Point) && (wantCounter)) ) 
               { 
                   ...
                   if(entermode == OP_BUYSTOP && StopOrder) {
                     ...
                     Print("In Second BuyStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long);
                   }
                   
                   if(entermode == OP_BUYLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,LongLots,traderate_Long,0,myStopLoss,traderate_Long+point*(GridSize+BaseTakeProfit),GridName,MagicNumberGrid,0,Green); 
                     Print("In Second OP_BUYLIMIT.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long);
                   }
               }
          }
            ...
            if( ((traderate_Long > Ask ) && ((traderate_Long-Ask) >= GridSize*Point) && (wantBreakout)) || ((traderate_Long <= Ask ) && ((Ask-traderate_Long) >= GridSize*Point) && (wantCounter)) ) 
               {  
                  ...
                  if(entermode == OP_BUYSTOP && StopOrder) {
                     ...
                     Print("In Third BuyStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long);
                  }
                   if(entermode == OP_BUYLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,LongLots,traderate_Long,0,myStopLoss,traderate_Long+point*(GridSize*2+BaseTakeProfit),GridName,MagicNumberGrid,0,Green); 
                     Print("In Third BuyLimit.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate_Long: "+traderate_Long); 
                   }
         ...
         if ( ProfitRank1 && ((CheckPositionRank & 1) == 0) )           // test if i have no open orders profit rank 1 close to my price: if so, put one on
          {
             ...
             if( ((traderate < Bid ) && ((Bid-traderate) >= GridSize*Point) && (wantBreakout)) || ((traderate >= Bid ) && ((traderate-Bid) >= GridSize*Point) && (wantCounter)) ) 
               { 
                  
                  if(entermode == OP_SELLSTOP && StopOrder) {
                     ...
                     Print("In First SellStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);
                  }
                  if(entermode == OP_SELLLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,ShortLots,traderate,0,myStopLoss,traderate-point*BaseTakeProfit,GridName,MagicNumberGrid,0,Red); 
                     Print("In First SellLimit.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);
                  }
               }
          }
             ...
             if( ((traderate < Bid ) && ((Bid-traderate) >= GridSize*Point) && (wantBreakout)) || ((traderate >= Bid ) && ((traderate-Bid) >= GridSize*Point) && (wantCounter)) ) 
               { 
                  ...
                  if(entermode == OP_SELLSTOP && StopOrder) {
                     ...
                     Print("In Second SellStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);
                  }
                  if(entermode == OP_SELLLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,ShortLots,traderate,0,myStopLoss,traderate-point*(GridSize+BaseTakeProfit),GridName,MagicNumberGrid,0,Red); 
                     Print("In Second SellLimit.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);
                  }
             ...
             if( ((traderate < Bid ) && ((Bid-traderate) >= GridSize*Point) && (wantBreakout)) || ((traderate >= Bid ) && ((traderate-Bid) >= GridSize*Point) && (wantCounter)) ) 
               { 
                  ...
                  if(entermode == OP_SELLSTOP && StopOrder) {
                     ...
                     Print("In Third SellStop.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);
                  }
                  if(entermode == OP_SELLLIMIT && LimitOrder) {
                     ticket=OrderSend(Symbol(),entermode,ShortLots,traderate,0,myStopLoss,traderate-point*(GridSize*2+BaseTakeProfit),GridName,MagicNumberGrid,0,Red); 
                     Print("In Third SellLimit.. wantBreakout: "+wantBreakout+"    wantCounter: "+wantCounter+"  traderate: "+traderate);  
                  }
