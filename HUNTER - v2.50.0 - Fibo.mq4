...
int Fibonacci(string Mode, int Number)
   {
       int Fibo = 0;
       if(Mode == "Seq")
         {
                if((Number == 1) || (Number == 2)) Fibo = 1;
                if(Number == 3) Fibo = 2;
                if(Number == 4) Fibo = 3;
                if(Number == 5) Fibo = 5;
                if(Number == 6) Fibo = 8;
...
//+------------------------------------------------------------------+
void OrderManagement(string Type, double SignalPrice, double NearbyResistance, double lot)
   {
      datetime RightBarTime = iTime(_Symbol,_Period,0);
      double OrderPrice;
      double Fibonized_Lot;
      double StopLoss;
      ...
      if(Type == "BUY")
        {
            StopLoss = Stop_Generator("BUY",Ask,1);
            gBuyTicket = OrderSend(_Symbol,OP_BUY,lot,Ask,100,StopLoss,0,NULL,0);
            ...
            for(int i=2;i<=PosNumber;i++)
               { 
                  OrderPrice = SignalPrice - (5*_Point);
                  OrderPrice = NormalizeDouble(OrderPrice,Digits);
                  Fibonized_Lot = lot*Fibonacci("Seq",i);
                  StopLoss = Stop_Generator("BUY",OrderPrice,i);
                  if(Ask > OrderPrice)   gBuyTicket = OrderSend(_Symbol,OP_BUYLIMIT,Fibonized_Lot,OrderPrice,100,StopLoss,0,NULL,Fibonacci("Seq",i),Expired);
                  if(Ask == OrderPrice)    gBuyTicket = OrderSend(_Symbol,OP_BUY,Fibonized_Lot,Ask,100,StopLoss,0,NULL,Fibonacci("Seq",i));
                  else if(Ask < OrderPrice)  gBuyTicket = OrderSend(_Symbol,OP_BUYSTOP,Fibonized_Lot,OrderPrice,100,StopLoss,0,NULL,Fibonacci("Seq",i),Expired);
                  
               }
        }
     if(Type == "SELL")
       {
            StopLoss = Stop_Generator("SELL",Bid,1);
            gSellTicket = OrderSend(_Symbol,OP_SELL,lot,Bid,100,StopLoss,0,NULL,0);
            ...
            for(int i=2;i<=PosNumber;i++)
               { 
                  ...
                  OrderPrice = NormalizeDouble(OrderPrice,Digits);
                  Fibonized_Lot = lot*Fibonacci("Seq",i);
                  StopLoss = Stop_Generator("SELL",OrderPrice,i);
                  if(Bid < OrderPrice)  gSellTicket = OrderSend(_Symbol,OP_SELLLIMIT,Fibonized_Lot,OrderPrice,100,StopLoss,0,NULL,Fibonacci("Seq",i),Expired);
                  if(Bid == OrderPrice)   gSellTicket = OrderSend(_Symbol,OP_SELL,Fibonized_Lot,Bid,100,StopLoss,0,NULL,Fibonacci("Seq",i));
                  else if(Bid > OrderPrice)  gSellTicket = OrderSend(_Symbol,OP_SELLSTOP,Fibonized_Lot,OrderPrice,100,StopLoss,0,NULL,Fibonacci("Seq",i),Expired);
                  
               }
       }
   }
//+------------------------------------------------------------------+   
double Stop_Generator(string Type, double OpenPrice, int Number)
   {
      double StopLossPrice;   
      if(Type == "BUY")  {StopLossPrice = OpenPrice - (EdgeUnit*_Point*Fibonacci("Rev",Number));  return StopLossPrice;}
      if(Type == "SELL") {StopLossPrice = OpenPrice + (EdgeUnit*_Point*Fibonacci("Rev",Number));  return StopLossPrice;} 
      return NULL;  
   }
                              ...
                              if(LastClosePrice > EMALine)
                                  {
                                    ...
                                    OrderManagement("BUY",Ask,KijunSen_Signal,LotOrder);
                              ...
                              else
                                {
                                    double LotOrder = Lot;
                                    if(DynamicLot == 1) LotOrder = LotGen();
                                    OrderManagement("BUY",Ask,KijunSen_Signal,LotOrder/2);
                                 ...
                                 if(LastClosePrice < EMALine)
                                   {
                                      double LotOrder = Lot;
                                      if(DynamicLot == 1) LotOrder = LotGen();
                                      OrderManagement("SELL",Bid,KijunSen_Signal,LotOrder);
                                 ...
                                 else
                                   {
                                      double LotOrder = Lot;
                                      if(DynamicLot == 1) LotOrder = LotGen();
                                      OrderManagement("SELL",Bid,KijunSen_Signal,LotOrder/2);
                                      gBearCross = 0;
                                      Comment(RightBarTime,Message,TrailingStep,PA,gPAProfit,gRegularProfit);
                                   }
                             }
                                       ...
                                       if((ChikouSpan < LowPrice) && (ChikouSpan < SenkouSpan_B_For_Chikou))
                                        {
                                            ...
                                            double LotOrder = Lot;
                                            if(DynamicLot == 1) LotOrder = LotGen();
                                            OrderManagement("SELL",Bid,Span_B_Signal,LotOrder);
                                       ...
                                       if((ChikouSpan > HighPrice) && (ChikouSpan > SenkouSpan_B_For_Chikou))
                                          {
                                              ...
                                              double LotOrder = Lot;
                                              if(DynamicLot == 1) LotOrder = LotGen();
                                              OrderManagement("BUY",Bid,Span_B_Signal,LotOrder);
...
//+------------------------------------------------------------------+ 
bool Shield_Zone(string Type, double OpenPrice, double StopLoss)
   {
       ...
       if(Type == "BUY")
           {
               ...
               if((StopLoss - OpenPrice) > 0) return false;
       ...
       else if(Type == "SELL")
           {
               ...
               if((OpenPrice - StopLoss) > 0) return false;
               if(Price_Movement >= Shield_Activation*_Point)  return true;
...
//+------------------------------------------------------------------+    
int Trailing_Step(int MagicNumber)
   {
      int GenTrailingStep = 0;
      if(MagicNumber == 0) GenTrailingStep = 500;
      if(MagicNumber == 1) GenTrailingStep = 300;
      if(MagicNumber == 2) GenTrailingStep = 200;
      if((MagicNumber == 3) || (MagicNumber == 5)) GenTrailingStep = 100;
      return GenTrailingStep;
                    ...
                    if(OrderSymbol() == Symbol())
                       {
                           if(OrderType() == OP_BUY)
                             {  
                                ...
                                if(Protection != 0)
                                  {
                                       if((Shield_Zone("BUY",OrderOpenPrice(),OrderStopLoss()) == True))
                                           {
                                ...
                                if(Bid - OrderOpenPrice() >= Trailing_Step(OrderMagicNumber())*_Point)
                                  {
                           ...
                           else if(OrderType() == OP_SELL)
                                  {
                                       ...
                                       if(Protection != 0)
                                         {
                                             if((Shield_Zone("SELL",OrderOpenPrice(),OrderStopLoss()) == True))
                                                 {
                                                        gModifiedTicket = OrderModify(OrderTicket(), OrderOpenPrice(), OrderOpenPrice() - (Shield*_Point),0,0,0);
                                                 }
                                         }
                                           
                                       if(OrderOpenPrice() - Ask >= Trailing_Step(OrderMagicNumber())*_Point)
                                         {
